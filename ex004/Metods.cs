...
public static void FillArrayDouble (double []NameArray, int min, int max)
{
  int index = 0;
...
public static void FindInArrayAndSummCol(int [] NameArray, int min, int max)
{
  int summ = 0;
  for (int i = 0; i < NameArray.Length; i++)
  {
    if (NameArray[i] > min && NameArray[i] < max) summ++;
  }
  Console.WriteLine(summ);
}

  public static int GetWays(int numberA, int numberB)
  {
    int summ = 0;
    int k1 = 1;
    int k2 = 2;
    if (numberA * k2 <= numberB && numberA + k1 <= numberB)
    {
      summ++;
      int A = numberA+k1;
      int B = numberA*k2;
      summ = summ + GetWays(A, numberB);
      summ = summ + GetWays(B, numberB);
      return summ;
    }
    else
    {
  ...
  public static void PrintPrevr(int A, int B)
  {
    Console.Write($"{A} +");
    for (int i = A+1; i < B; i++)
  {
